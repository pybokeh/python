...
if action == "update":
    ...
    final_sql = sql + "'" + str(stopdt) + "' and flag = 'True' GROUP BY \"datLastRun\", \"txtFactory\""

    let.execute(final_sql)
    lastrun = let.fetchall()

    for row in lastrun:
        datLastRun = row[0]
        plant = row[1]
        while datLastRun < stopdt:
            let.execute("""SELECT "txtPath" FROM let_paths WHERE "txtPlant" = '""" + plant + "'")
            base = let.fetchone()
            ...
            let.execute("""SELECT * FROM let_tests WHERE "datLastRun" = '""" + str(datLastRun) + """' AND "txtFactory" = '"""+ plant + "'")
            result = let.fetchall()
