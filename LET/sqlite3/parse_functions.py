...
def load_lwc_angles_ver2(file):
    ...
    grade_short  = features[6]
    grade_long   = features[7]
    dest_cd      = features[8]    
    ...
    if not process == None and not (process.find('test_attribute') == None):
        ...
        #Insert the values into the table, if there is a dup go ahead and continue
        try:
            LET.execute("insert into lwc(txtvin,pitch_offset,yaw_offset,roll_offset,dattime,mdl_yr,fctry_cd,mdl_nm,doors,eng_cyl,transmission,grade_short,grade_long,dest_cd) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(vin,pitch,yaw,roll,time,year,factory,model,doors,eng_cyl,transmission,grade_short,grade_long,dest_cd))
        except (sqlite3.IntegrityError):
            pass
...
def getVinFeatures(txtVIN):
    ...
    while datetime.now().hour > 20 or datetime.now().hour < 7:
...
def test_connections():
    ...
    print "Testing connection to sqlite LET: ",
    print "",
    #Need to test availability of the LET Database
    if (pf.test_sqlite("denso") == 1):
        print "OK"
    else:
        print "FAILED"
    ...
    print ""
    print "Testing connection to Sqlite3: ",
    print "",
    #Need to test availability of the SQL Warranty database
    if (pf.test_sqlite("denso") == 1):
        print "OK"
        ...
        print "FAILED"
        sys.exit("Could not connect to Sqlite3 database")
    print ""    
